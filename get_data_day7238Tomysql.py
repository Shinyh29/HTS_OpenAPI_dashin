# short_amount 공매도거래대금  의 item_tb ( keys ) 2 db ec2
# 7238
import pymysql
...
#import os
#os.system('python C:\\PycharmProjects\\dashin_api\\window_control_main.py')

for idx, ticker in enumerate(stocks):
    unit_df = pd.DataFrame()
    df = get_data_day7238.get_diff7238(code=ticker) #'20000101')
    df = df.reset_index()
    df['Ticker'] = ticker

    table_nms = df.keys().tolist()
    for table_nm in table_nms:
        if table_nm in ['index','Ticker','Date']: #, 'netbuy_instit']:
